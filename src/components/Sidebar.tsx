    ...
    <aside className="w-sidebar flex-shrink-0 bg-primary flex flex-col h-full">
      ...
      <div className="border-t border-primary-light px-3 py-3 flex-shrink-0 space-y-2">
        {/* <button
          ...
          onClick={async () => {
            try {
              await window.bilpow.devtools.open();
            } catch (err) {
              console.error('Failed to open DevTools:', err);
            }
          }}
          className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-blue-200 hover:bg-primary-light hover:text-white transition-colors"
        >
          <span>🔧</span>
          Console Dev
        </button> */}
